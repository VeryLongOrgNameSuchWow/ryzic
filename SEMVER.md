...
- **Environment variable names and meanings.** The configuration contract is `DISCORD_BOT_TOKEN`, `LAVALINK_HOST`, `LAVALINK_PORT`, `LAVALINK_PASSWORD`, `RYZIC_CACHE_DIR`, `RYZIC_CACHE_MAX_GB`, `RYZIC_LOG_LEVEL`, `RYZIC_GUILD_IDS`, `RYZIC_AUTOLEAVE_SECONDS`, and `RYZIC_YOUTUBE_COOKIES_PATH`. Renaming any of them, or changing the units of `RYZIC_CACHE_MAX_GB` or `RYZIC_AUTOLEAVE_SECONDS`, is breaking. Adding new optional env vars with sensible defaults is not.
